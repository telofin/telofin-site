...
function rb(type,i){
  ...
  if(item.reconciled){
    return'<div class="row-acts">'
      ...
      +'<span style="font-size:13px;color:var(--muted)" title="Reconciled — edit disabled">🔒</span>'
      +'</div>';
  }
  return'<div class="row-acts">'
    +(hasAudit?'<button class="e-btn" onclick="openTxnAuditLog(\''+type+'\','+i+')" title="Edit history">&#128221;</button>':'')
    +'<button class="e-btn" onclick="editItem(\''+type+'\','+i+')" title="Edit">&#9998;</button>'
    +'<button class="e-btn" onclick="voidItem(\''+type+'\','+i+')" title="Void & reverse" style="color:var(--amber)">⊘</button>'
    +'<button class="d-btn" onclick="delItem(\''+type+'\','+i+')" title="Delete">&#215;</button>'
    +'</div>';
}
...
function renderAll(force){
  ...
  if(c.type==='np'){safe(function(){renderGrants(c);},'grants');safe(function(){renderProcurement(c);},'procurement');safe(function(){renderNpInc(c);},'funding');safe(function(){renderNpExp(c);},'npexp');safe(function(){renderDonors(c);},'donors');safe(function(){renderNpJrn(c);},'npjrn');safe(function(){renderNpAct(c);},'npact');var reconP=g('p-recon');if(reconP&&reconP.classList.contains('active'))safe(function(){renderReconciliation(c);},'recon');}
  else if(c.type==='sb'){safe(function(){renderRev(c);},'revenue');safe(function(){renderCF(c);},'cashflow');safe(function(){renderSbExp(c);},'sbexp');safe(function(){renderAR(c);},'ar');safe(function(){renderJournalEntries(c);},'je');safe(function(){renderBalanceSheet(c);},'bs');var reconPsb=g('p-recon');if(reconPsb&&reconPsb.classList.contains('active'))safe(function(){renderReconciliation(c);},'recon');safe(function(){renderSbJrn(c);},'sbjrn');safe(function(){renderSbAct(c);},'sbact');}
...
function renderGrants(cc){
  ...
  var spent=gExp.reduce(function(s,e){return s+Number(e.amt||0);},0),awarded=Number(gr.awarded||0),rem=awarded-spent;
  var incRecv=gInc.reduce(function(s,r){return s+Number(r.recv||0);},0);
  var eRows=gExp.length?gExp.map(function(e){var oi=(c.expenses||[]).indexOf(e);return'<tr><td>'+escHtml(e.desc)+'</td><td>'+escHtml(e.cat||'—')+'</td><td>'+fmt(e.amt)+'</td><td style="color:var(--muted)">'+(e.date||'—')+'</td><td><input type="checkbox" class="rcb" '+(e.reconciled?'checked':'')+' onchange="tgRecon('+oi+')" title="Reconciled"></td><td>'+rb('expenses',oi)+'</td></tr>';}).join(''):'<tr><td colspan="6" style="text-align:center;padding:1.5rem;color:var(--muted);font-size:12px">No expenses logged against this grant yet.</td></tr>';
  var iRows=gInc.length?gInc.map(function(r){var oi=(c.income||[]).indexOf(r);return'<tr><td style="font-weight:500">'+escHtml(r.name||'—')+'</td><td>'+escHtml(r.cat||'—')+'</td><td class="vb">'+fmt(r.proj)+'</td><td class="vg">'+fmt(r.recv)+'</td><td style="color:var(--muted)">'+(r.date||'—')+'</td><td>'+SB(r.status||'')+'</td><td>'+rb('income',oi)+'</td></tr>';}).join(''):'<tr><td colspan="7" style="text-align:center;padding:1.5rem;color:var(--muted);font-size:12px">No income entries linked to this grant yet. Click "+ Link income" above to add one.</td></tr>';
  ...
  +(function(){
    ...
    if(unrecon>0)gaps.push({sev:'amber',msg:unrecon+' unreconciled expense'+(unrecon===1?'':'s')});
...
function tgRecon(i){var c=gc();if(!c||!c.expenses[i])return;c.expenses[i].reconciled=!c.expenses[i].reconciled;sv();renderGrants();renderNpExp(c);}
function tgReconInc(i){var c=gc();if(!c||!c.income[i])return;c.income[i].reconciled=!c.income[i].reconciled;sv();renderNpInc(c);}
...
function renderNpExp(c){
  ...
  var rows=filt.map(function(r){var oi=exp.indexOf(r);var acctE=(c.accounts||[]).find(function(a){return a.code===r.acctCode||a.cat===r.cat||a.name===r.cat;});var codeE=acctE?'<span style="font-family:monospace;font-size:10px;background:var(--soft);padding:1px 4px;border-radius:3px">'+acctE.code+'</span>':'—';var _vnd=r.vendor1099||r.desc||'';var _descCell=_vnd?drillLink(_vnd,'vendor',_vnd,'vendor'):escHtml(r.desc||'—');var _catCell=r.cat?drillLink(r.cat,'cat',r.cat,'expdetail'):('—');var _fundCell=r.fund?'<span class="badge b-green" style="cursor:pointer" onclick="runItemReport(\'fund\',\''+escHtml(r.fund)+'\',\'Fund: '+escHtml(r.fund)+'\',\'expdetail\')" title="Run fund report">'+escHtml(r.fund)+'</span>':(r.grantId?'<span class="badge b-green">Grant</span>':'—');return'<tr'+(r.voided?' style="opacity:.55;text-decoration:line-through"':'')+'><td>'+_descCell+'</td><td>'+_catCell+'</td><td>'+codeE+'</td><td>'+fmt(r.amt)+'</td><td style="color:var(--muted)">'+(r.date||'—')+'</td><td style="color:var(--muted);font-size:11px">'+(r.checkNum||'—')+'</td><td>'+rcptCell('expenses',oi,r)+'</td><td>'+_fundCell+'</td><td><input type="checkbox" class="rcb" '+(r.reconciled?'checked':'')+' onchange="tgRecon('+oi+')" title="Reconciled"></td><td>'+(r.inkindRef?'<span style="font-size:10px;color:var(--muted)" title="Auto-created from in-kind donation">&#9889; system</span>':'<span style="display:inline-flex;align-items:center;gap:3px">'+(r.functionalSplit?'<span class="badge b-blue" style="font-size:10px" title="Split allocation">split</span>':'')+rb('expenses',oi)+'<button class="e-btn" onclick="openAllocModal('+oi+')" title="Allocate across functional categories" style="font-size:10px">&#8853;</button></span>')+'</td></tr>';}).join('');
  var cs=catSum(exp,'amt');
  p.innerHTML=FB()+XB('expenses')+renderPayroll(c)+renderAP(c)+renderCC(c)+catF(exp,'renderNpExp(gc())')+srchBar('p-npexp','renderNpExp(gc())')+'<div class="metrics"><div class="metric"><div class="m-lbl">Total expenses</div><div class="m-val vr">'+fmt(tot)+'</div></div></div>'+(cs?'<div class="card"><div class="c-title" style="margin-bottom:.75rem">By category</div>'+cs+'</div>':'')+'<div class="card"><div class="c-head"><span class="c-title">Expense log</span><button class="add-btn" onclick="EI=-1;g(\'e-gid\').value=\'\';openM(\'m-exp\')">+ Add expense</button></div>'+(exp.length?'<table><thead><tr><th style="width:16%">Description</th><th style="width:9%">Category</th><th style="width:7%">Code</th><th style="width:8%">Amount</th><th style="width:8%">Date</th><th style="width:7%">Check #</th><th style="width:5%">Receipt</th><th style="width:9%">Fund</th><th style="width:9%">Recon</th><th style="width:21%"></th></tr></thead><tbody>'+rows+'</tbody></table>':ES('No expenses yet','Log your first expense.','EI=-1;g(\'e-gid\').value=\'\';openM(\'m-exp\')'))+'</div>';
}
...
function renderSbExp(c){
  ...
  var rows=filt.map(function(r){var oi=exp.indexOf(r),mo2=r.freq==='Weekly'?r.amt*4:r.freq==='Bi-weekly'?r.amt*2:r.freq==='Monthly'?r.amt:r.freq==='Quarterly'?Math.round(r.amt/3):r.freq==='Annual'?Math.round(r.amt/12):r.amt;var acctSb=(c.accounts||[]).find(function(a){return a.code===r.acctCode||a.cat===r.cat||a.name===r.cat;});var codeSb=acctSb?'<span style="font-family:monospace;font-size:10px;background:var(--soft);padding:1px 4px;border-radius:3px">'+acctSb.code+'</span>':'—';var _vnd=r.vendor1099||r.desc||'';var _descCell=_vnd?drillLink(_vnd,'vendor',_vnd,'vendor'):escHtml(r.desc||'—');var _catCell=r.cat?drillLink(r.cat,'cat',r.cat,'expdetail'):('—');return'<tr><td>'+_descCell+'</td><td>'+_catCell+'</td><td>'+codeSb+'</td><td>'+fmt(r.amt)+'</td><td style="color:var(--muted)">'+(r.freq||'—')+'</td><td>'+fmt(mo2)+'</td><td><span class="badge '+(r.fixed==='Fixed'?'b-blue':'b-amber')+'">'+(r.fixed||'')+'</span>'+(r.recurring&&r.recurring!=='None'?' <span class="badge b-rec">&#8635;</span>':'')+'</td><td style="color:var(--muted);font-size:11px">'+(r.checkNum||'—')+'</td><td>'+rcptCell('expenses',oi,r)+'</td><td><input type="checkbox" class="rcb" '+(r.reconciled?'checked':'')+' onchange="sbRC('+oi+')" title="Reconciled"></td><td>'+rb('expenses',oi)+'</td></tr>';}).join('');
  var cs=catSum(exp,'amt');
  p.innerHTML=FB()+XB('expenses')+renderPayroll(c)+renderAP(c)+renderCC(c)+catF(exp,'renderSbExp(gc())')+srchBar('p-sbexp','renderSbExp(gc())')+'<div class="metrics"><div class="metric"><div class="m-lbl">Monthly burn</div><div class="m-val vr">'+fmt(Math.round(mo))+'</div></div><div class="metric"><div class="m-lbl">Fixed costs</div><div class="m-val">'+fmt(fx)+'</div></div><div class="metric"><div class="m-lbl">Variable costs</div><div class="m-val">'+fmt(vr)+'</div></div></div>'+(cs?'<div class="card"><div class="c-title" style="margin-bottom:.75rem">By category</div>'+cs+'</div>':'')+'<div class="card"><div class="c-head"><span class="c-title">Expenses</span><button class="add-btn" onclick="EI=-1;openM(\'m-exp\')">+ Add expense</button></div>'+(exp.length?'<table><thead><tr><th style="width:13%">Description</th><th style="width:8%">Category</th><th style="width:7%">Code</th><th style="width:7%">Amount</th><th style="width:7%">Freq</th><th style="width:7%">Monthly</th><th style="width:8%">Type</th><th style="width:7%">Check #</th><th style="width:5%">Receipt</th><th style="width:7%">Recon</th><th style="width:24%"></th></tr></thead><tbody>'+rows+'</tbody></table>':ES('No expenses yet','Add your business expenses.','EI=-1;openM(\'m-exp\')'))+'</div>';
}
function sbRC(i){var c=gc();if(!c||!c.expenses[i])return;c.expenses[i].reconciled=!c.expenses[i].reconciled;sv();renderSbExp(c);}
...
function renderPeExp(c){
  ...
  var tot=exp.reduce(function(s,r){return(r.voided||r.isReversal)?s:s+Number(r.amt||0);},0);
  var rows=filt.map(function(r){var oi=exp.indexOf(r);var acctPE=(c.accounts||[]).find(function(a){return a.code===r.acctCode||a.cat===r.cat||a.name===r.cat;});var codePE=acctPE?'<span style="font-family:monospace;font-size:10px;background:var(--soft);padding:1px 4px;border-radius:3px">'+acctPE.code+'</span>':'—';var _vnd=r.vendor1099||r.desc||'';var _descCell=_vnd?drillLink(_vnd,'vendor',_vnd,'vendor'):escHtml(r.desc||'—');var _catCell=r.cat?drillLink(r.cat,'cat',r.cat,'expdetail'):('—');return'<tr><td>'+_descCell+'</td><td>'+_catCell+'</td><td>'+codePE+'</td><td>'+fmt(r.amt)+'</td><td style="color:var(--muted)">'+(r.freq||'—')+(r.recurring&&r.recurring!=='None'?' <span class="badge b-rec">&#8635;</span>':'')+'</td><td style="color:var(--muted)">'+(r.date||'—')+'</td><td style="color:var(--muted);font-size:11px">'+(r.checkNum||'—')+'</td><td>'+rcptCell('expenses',oi,r)+'</td><td><input type="checkbox" class="rcb" '+(r.reconciled?'checked':'')+' onchange="peRC('+oi+')" title="Reconciled"></td><td>'+rb('expenses',oi)+'</td></tr>';}).join('');
  var cs=catSum(exp,'amt');
  p.innerHTML=FB()+XB('expenses')+catF(exp,'renderPeExp(gc())')+srchBar('p-peexp','renderPeExp(gc())')+'<div class="metrics"><div class="metric"><div class="m-lbl">Total expenses</div><div class="m-val vr">'+fmt(tot)+'</div></div></div>'+(cs?'<div class="card"><div class="c-title" style="margin-bottom:.75rem">By category</div>'+cs+'</div>':'')+'<div class="card"><div class="c-head"><span class="c-title">Expense log</span><button class="add-btn" onclick="EI=-1;openM(\'m-exp\')">+ Add expense</button></div>'+(exp.length?'<table><thead><tr><th style="width:16%">Description</th><th style="width:10%">Category</th><th style="width:7%">Code</th><th style="width:8%">Amount</th><th style="width:10%">Frequency</th><th style="width:10%">Date</th><th style="width:7%">Check #</th><th style="width:5%">Receipt</th><th style="width:7%">Recon</th><th style="width:20%"></th></tr></thead><tbody>'+rows+'</tbody></table>':ES('No expenses yet','Start logging your household expenses.','EI=-1;openM(\'m-exp\')'))+'</div>';
}
function peRC(i){var c=gc();if(!c||!c.expenses[i])return;c.expenses[i].reconciled=!c.expenses[i].reconciled;sv();renderPeExp(c);}
...
function saveInc(){var c=gc();if(!c.income)c.income=[];
  ...
  var _iExpCode=item.acctCode||_defaultARCode(c);var _iCrCode=item.acctCode||'4010';if(_rEI>=0)updateLedgerEntry(c,item.id||c.income[_rEI].id,_defaultARCode(c),_iExpCode,Number(item.recv||0),item.name||'Income','income');else postToLedger(c,_defaultCashCode(c),item.acctCode||'4010',Number(item.recv||0),item.name||'Income','income',item.id);markDirty('funding','budget','reports','bs');if(_rEI>=0)c.income[_rEI]=item;else c.income.push(item);sv();renderNpInc(c);renderBudgetMultiYear();renderReports();renderBalanceSheet(c);if(item.grantId)renderGrants(c);closeM('m-inc');['i-n','i-c','i-p','i-r','i-dt','i-fund','i-gid'].forEach(function(id){var el=g(id);if(el)el.value='';}); }
function saveRev(){var c=gc();if(!c.revenue)c.revenue=[];
  ...
  if(item.recurring&&item.recurring!=='None'){if(_rRecurEnd)item.recurEndDate=_rRecurEnd;if(_rRecurCnt>0){item.recurCount=_rRecurCnt;item.recurPostedCount=0;}}
  ...
  markDirty('revenue','budget','reports','bs');if(_rREI>=0)c.revenue[_rREI]=item;else c.revenue.push(item);sv();renderRev(c);renderBudgetMultiYear();renderReports();renderBalanceSheet(c);closeM('m-rev');['r-n','r-c','r-p','r-a','r-dt','r-taxrate','r-taxamt'].forEach(function(id){var el=g(id);if(el)el.value='';});if(g('r-taxrate'))g('r-taxrate').value='0';if(g('r-taxjur'))g('r-taxjur').value=''; }
function revTaxCalc(){
