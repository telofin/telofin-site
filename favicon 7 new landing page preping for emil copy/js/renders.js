...
function tgRecon(i){var c=gc();if(!c||!c.expenses[i])return;var cur=c.expenses[i].reconciled;if(!cur&&!confirm('Mark this transaction as reconciled?'))return;if(cur&&!confirm('Unmark this transaction as reconciled?'))return;c.expenses[i].reconciled=!cur;sv();renderGrants();renderNpExp(c);}
...
function saveRev(){var c=gc();if(!c.revenue)c.revenue=[];
  ...
  markDirty('revenue','budget','reports','bs');if(_rREI>=0)c.revenue[_rREI]=item;else c.revenue.push(item);sv();renderRev(c);renderBudgetMultiYear();renderReports();renderBalanceSheet(c);closeM('m-rev');['r-n','r-c','r-p','r-a','r-dt','r-taxrate','r-taxamt','r-cust'].forEach(function(id){var el=g(id);if(el)el.value='';});if(g('r-taxrate'))g('r-taxrate').value='0';if(g('r-taxjur'))g('r-taxjur').value=''; }
function revTaxCalc(){
