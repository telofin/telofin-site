...
function showRunReportMenu(btn,panelType){
  ...
  var items=[
    {label:'Transaction Report',icon:'<i class="fas fa-clipboard"></i>',fn:'clearRptFilter();runItemReport(\'\',\'\',\'\',\''+txnTarget+'\')'},
    {label:'P&amp;L',icon:'<i class="fas fa-chart-column"></i>',fn:'clearRptFilter();runItemReport(\'\',\'\',\'\',\'pl\')'},
    {label:'Balance Sheet',icon:'<i class="fas fa-scale-balanced"></i>',fn:'clearRptFilter();runItemReport(\'\',\'\',\'\',\'bsheet\')'}
  ];
  if(panelType==='grants'&&isNP){items.unshift({label:'Grant Close-Out Report',icon:'<i class="fas fa-file"></i>',fn:'runItemReport(\'\',\'\',\'\',\'grantcloseout\')'},{label:'Grant Status Report',icon:'<i class="fas fa-chart-column"></i>',fn:'runItemReport(\'\',\'\',\'\',\'grantstatus\')'});}
...
function showRowActionsMenu(btn,type,i){
  ...
  if(type==='expenses'&&item.bankId&&!item.checkPrinted)items.push({label:'Print check',icon:'<i class="fas fa-print"></i>',fn:'printCheckForExpense('+i+')'});
...
// printCheckForExpense(i): prints a check for a plain expense entered via the generic
// Expense modal (not through Bills) that already has a bank account + check/ref number set
// (e-bank/e-ref, saveExp()) but has never actually been printed — i.e. checkPrinted is not
// yet set, distinct from reprintCheck()'s condition. Mirrors _payOneBill()'s post-print
// bookkeeping (checkPrinted flag, nextCheckNum advance) and confirmPayBill()'s calibration
// checkpoint, without touching the ledger — the expense is already posted, this only adds
// the missing print action.
function printCheckForExpense(i){
  var c=gc();var item=c&&c.expenses&&c.expenses[i];
  if(!item||!item.bankId)return;
  var bank=(c.bankAccounts||[]).find(function(b){return b.id===item.bankId;});
  if(!bank){alert('Could not find the bank account this expense is paid from. If that account was renamed or removed, set up "Check settings" again from the Reconciliation tab.');return;}
  if(!bank.checkCalibrated){
    if(confirm('You haven\'t printed a test alignment page for "'+bank.name+'" yet.\n\nOK = print a free test page first (nothing else happens — come back and use "Print check" again when it lines up)\nCancel = skip this and print the real check now')){
      printCheckAlignmentTest(bank.checkOffsetX||0,bank.checkOffsetY||0,bank.checkFormat||'voucher2');
      return;
    }
  }
  var checkNum=(item.checkNum||'').trim();
  if(!checkNum){
    checkNum=(prompt('Enter the check number:',String(bank.nextCheckNum||''))||'').trim();
    if(!checkNum)return;
  }
  var slot=1;
  if(bank.checkFormat==='standard3'){
    var slotIn=(prompt('Which check slot is blank on the sheet? Enter 1 (top), 2 (middle), or 3 (bottom):','1')||'1').trim();
    slot=[1,2,3].indexOf(Number(slotIn))>=0?Number(slotIn):1;
  }
  var vendorRec=(c.vendors||[]).find(function(v){return v.name.toLowerCase()===(item.vendor1099||'').toLowerCase();});
  printCheck(c,bank,{payee:item.vendor1099||item.desc||'',address:vendorRec&&vendorRec.address||'',amount:item.amt,date:item.date,memo:item.desc||'',checkNum:checkNum,slot:slot});
  item.checkNum=checkNum;
  item.checkPrinted=true;
  var bankIdx=c.bankAccounts.indexOf(bank);
  var numPart=parseInt(checkNum,10);
  if(bankIdx>=0&&!isNaN(numPart))c.bankAccounts[bankIdx].nextCheckNum=numPart+1;
  sv();renderAll(true);
...
function rcptCell(col,oi,r){
  ...
  var clip=hasFile
    ?'<button class="e-btn" onclick="vaultViewReceipt(\''+col+'\','+oi+')" title="View receipt" style="font-size:13px;color:var(--accent)"><i class="fas fa-paperclip"></i></button>'
    :'<button class="e-btn" onclick="vaultAttachReceipt(\''+col+'\','+oi+')" title="Attach receipt" style="font-size:13px;color:var(--muted)"><i class="fas fa-paperclip"></i></button>';
  return clip;
}
...
function billRcptCell(oi,b){
  ...
  return hasFile
    ?'<button class="e-btn" onclick="vaultViewBillInvoice('+oi+')" title="View invoice" style="font-size:13px;color:var(--accent)"><i class="fas fa-paperclip"></i></button>'
    :'<button class="e-btn" onclick="vaultAttachBillInvoice('+oi+')" title="Attach invoice" style="font-size:13px;color:var(--muted)"><i class="fas fa-paperclip"></i></button>';
}
function escHtml(s){
...
// Appends the sheet(s) for one tab type to an existing workbook. Returns true if it added any.
// Shared by exportTab (single tab) and exportAllData (whole client).
function _appendTabSheet(wb,type,c){
  var rows,ws;
  if(type==='expenses'){
...
function exportTab(type){
  ...
  if(!_appendTabSheet(wb,type,c)){return doXL();}
  XLSX.writeFile(wb,(c.name||'client').replace(/[^a-z0-9]/gi,'-')+'-'+type+'-clarity.xlsx');
}
// Export EVERYTHING for the current client — one workbook, a sheet per tab (client-type
// aware). Backs the free-tier data-safety "Export to Excel" button.
function exportAllData(){
  var c=gc();if(!c){alert('Open a client first to export its data.');return;}
  var types=c.type==='sb'?['revenue','expenses','budget','gl','ar','procurement']
           :c.type==='pe'?['peinc','expenses','budget','gl']
           :['income','expenses','donors','grants','budget','gl','ar','procurement'];
  var wb=XLSX.utils.book_new();
  types.forEach(function(t){try{_appendTabSheet(wb,t,c);}catch(e){console.warn('[export] '+t+' failed:',e);}});
  if(!wb.SheetNames||!wb.SheetNames.length){alert('Nothing to export yet.');return;}
  XLSX.writeFile(wb,(c.name||'client').replace(/[^a-z0-9]/gi,'-')+'-all-data-clarity.xlsx');
}
function FB(){return'';}
function ES(title,sub,fn){return'<div class="empty"><div class="et">'+title+'</div><div class="es">'+sub+'</div>'+(fn?'<button class="add-btn" onclick="'+fn+'" style="margin:0 auto">+ Add one now</button>':'')+'<button class="fb-nudge" onclick="openFB()"><i class="fas fa-comment"></i> What do you wish we had? Drop us a line.</button></div>';}
...
function renderAll(force){
  ...
  // Bank tab isn't tied to a single client type (np/sb/pe all share it via
  // bank.js's getTabs() patch), so it's checked once here rather than in
  // each type branch above — same active-tab guard already used for
  // p-cc/p-recon. This matters because _bankFindMatchCandidates() only
  // re-evaluates at renderBank() time: without this, saving a bill payment,
  // payroll run, loan payment, or any other code path that calls
  // renderAll() wouldn't refresh pending bank-match suggestions for a user
  // already sitting on that tab.
  var _bankPanelActive=g('p-bank');
  if(_bankPanelActive&&_bankPanelActive.classList.contains('active'))safe(function(){renderBank(c);},'bank');
  renderRecurBanner();
...
function renderGrants(cc){
  ...
  function dlBadge(dateStr,label){
    ...
    if(days<0)return'<span class="badge" style="background:var(--red-bg);color:var(--red);border:1px solid var(--red)"><i class="fas fa-circle"></i> MISSED — '+Math.abs(days)+'d ago</span>';
    if(days<=5)return'<span class="badge" style="background:#fff0f0;color:#c0392b;border:1px solid #c0392b;font-weight:600"><i class="fas fa-circle"></i> '+label+' in '+days+'d</span>';
    if(days<=15)return'<span class="badge" style="background:#fff3e0;color:#e65100;border:1px solid #e65100"><i class="fas fa-triangle-exclamation"></i> '+label+' in '+days+'d</span>';
    if(days<=30)return'<span class="badge" style="background:var(--amber-bg);color:var(--amber);border:1px solid var(--amber)"><i class="fas fa-triangle-exclamation"></i> '+label+' in '+days+'d</span>';
  ...
  +(function(){
    ...
    if(!gaps.length)return'<div style="font-size:12px;color:var(--green);margin-top:.75rem;padding:.5rem 0"><i class="fas fa-check"></i> No compliance gaps</div>';
...
function _renderGrantReqList(reqs){
  ...
  el.innerHTML=reqs.map(function(r,i){
    return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">'
      ...
      +'onclick="_removeGrantReq('+i+')"><i class="fas fa-xmark"></i></button>'
      +'</div>';
  }).join('');
}
...
function renderDonors(cc){
  ...
  if(schedBDonors.length){
    ...
    schedBAlert='<div style="margin-bottom:1rem;padding:.875rem 1rem;background:#fff8e1;border:1px solid #f9a825;border-radius:10px;display:flex;gap:10px;align-items:flex-start">'
      +'<span style="font-size:18px;flex-shrink:0"><i class="fas fa-triangle-exclamation"></i></span>'
  ...
  if(totalInkindFMV>=25000){
    schedMAlert='<div style="margin-bottom:1rem;padding:.875rem 1rem;background:#e8f5e9;border:1px solid #43a047;border-radius:10px;display:flex;gap:10px;align-items:flex-start">'
      +'<span style="font-size:18px;flex-shrink:0"><i class="fas fa-clipboard"></i></span>'
  ...
  var cards=visibleDonors.map(function(d,_vi){
    ...
    var schedBBadge=dFyTot>=SCHED_B_THRESHOLD?'<span class="badge" style="background:#fff8e1;color:#f57f17;border:1px solid #f9a825;margin-left:6px;font-size:9px"><i class="fas fa-triangle-exclamation"></i> Sch. B</span>':'';
    ...
    var lapsedFlag=daysSinceGift!==null&&daysSinceGift>365?'<span class="badge b-amber" style="font-size:9px"><i class="fas fa-triangle-exclamation"></i> Lapsed '+Math.floor(daysSinceGift/365)+'yr</span>':'';
    ...
    var wpBadge=d.constituentType==='workplace'?'<span class="badge" style="background:#e8f5e9;color:#2e7d32;font-size:9px"><i class="fas fa-building"></i> Payroll</span>':'';
    // Tier badge
    ...
    return'<div class="donor-card">'
      ...
      +(pendingFollowups.length?'<span class="badge b-blue" style="margin-top:4px;display:inline-block"><i class="fas fa-calendar"></i> '+pendingFollowups.length+' follow-up'+(pendingFollowups.length>1?'s':'')+'</span>':'')
      +'</div></div>'
      ...
      +'<button class="add-btn" style="background:var(--blue-bg);color:var(--blue);border-color:var(--blue)" onclick="openActivityModal('+di+')"><i class="fas fa-clipboard"></i> Log activity</button>'
      +'<button class="e-btn" style="border:1px solid var(--border);border-radius:7px;padding:5px 10px;font-size:12px" onclick="openAnnualLetter('+di+')" title="Generate year-end consolidated acknowledgment letter"><i class="fas fa-file"></i> Year-end letter</button>'
      +(d.constituentType==='workplace'?'<button class="e-btn" style="border:1px solid var(--border);border-radius:7px;padding:5px 10px;font-size:12px" onclick="openWorkplaceGivingLetter('+di+')" title="Generate stewardship letter for workplace giving donor"><i class="fas fa-building"></i> Stewardship letter</button>':'')
...
function saveDonor(){
  ...
  Object.keys(keyDates).forEach(function(f){
    ...
    if(existingTypes.indexOf(label)<0){
      var _newMilestone={id:uid(),type:label,date:keyDates[f],notes:'',created:new Date().toISOString()};
      item.milestones.push(_newMilestone);
      if(typeof dwUpsertMilestone==='function')dwUpsertMilestone(c,item,_newMilestone);
    } else {
      // Update existing milestone date if it changed
      var existing=item.milestones.find(function(m){return m.type===label;});
      if(existing){existing.date=keyDates[f];if(typeof dwUpsertMilestone==='function')dwUpsertMilestone(c,item,existing);}
    }
  });
  ...
  if(typeof dwUpsertDonor==='function')dwUpsertDonor(c,item);
...
function _postDonationLedger(c,di,record,cashOverride){
  ...
  // cashOverride lets the Bank Deposit builder route a gift into Undeposited Funds
  // (1050) instead of straight to the bank; unset everywhere else, so the normal
  // direct-to-bank donation path is unchanged.
  var cashCode=cashOverride||_defaultCashCode(c);
  ...
  if(record.inkind==='Yes'){
    ...
    // Resolved by name (not hardcoded '5400', which is actually "Marketing & outreach" in NP's
    // default COA — a real miscoding bug, this was landing in Marketing every time instead of
    // its own line) — mirrors NP's own default "5410 In-kind expense" account by name, and
    // survives a client's COA being renumbered/healed the same way the loan/asset accounts do.
    if(!c.expenses)c.expenses=[];
    var inkindExpCode=_ensureDedicatedCOA(c,'In-kind expense','Expense','In-Kind Expense');
    var expItem={id:uid(),desc:inkindDesc,cat:'In-Kind Expense',amt:inkindAmt,date:record.date,fund:record.fund||'',functional:record.inkindFunctional||'fundraising',acctCode:inkindExpCode,inkindRef:true,reconciled:true,audit:[]};
    c.expenses.push(expItem);
    if(typeof dwUpsertExpense==='function')dwUpsertExpense(c,expItem);
    record.expenseRef=expItem.id;
    if(typeof postToLedger==='function')postToLedger(c,inkindExpCode,cashCode,inkindAmt,'In-kind expense: '+inkindDesc,'expense',expItem.id);
    ...
    if(typeof dwUpsertIncome==='function')dwUpsertIncome(c,incItem2);
...
// True if the given account code is (or resolves to) a donations account — the signal that
// an income entry represents a gift from a donor, not program/grant/other revenue. Matches
// the app's default COA cat label plus common variants real clients end up on (e.g. a
// custom "Donations" account auto-created by syncBudgetToCOA from a budget line, which is
// what CB PTA's real data actually uses once their default "Individual donations" account
// was deactivated) — same set of labels bank.js already treats as a donation category.
// Falls back to code 4010 the same way _defaultCashCode falls back to 1010.
function _isDonationAcct(c,code){
  if(!code)return false;
  var a=(c.accounts||[]).find(function(x){return x.code===code;});
  if(a){
    var cat=(a.cat||'').toLowerCase();
    return cat==='individual donations'||cat==='individual donation'||cat==='donations'||cat==='donation';
  }
  return code==='4010';
}
// Mirrors an income entry onto the Donors tab — the reverse direction of
// _postDonationLedger/_syncDonationLedger. Called from saveInc() when the bookkeeper explicitly
// sets "Link to: Donor" (i-party-type) — not inferred from the account chosen, since account
// labels drift on real client data (see _isDonationAcct, still used by the one-time healer for
// historical entries that predate this field). Looks up any existing link by the donation's own
// incomeRef first (not item.donorId), since saveInc() rebuilds a fresh item object from form
// fields on every save and doesn't carry donorId/donationRef forward — searching by incomeRef is
// what makes this idempotent regardless.
function _syncIncomeToDonorTab(c,item){
  if(!c||c.type!=='np'||!item)return;
  var donorName=(item.name||'').trim();
  if(!donorName)return;
  if(!c.donors)c.donors=[];
  var donor=null,dn=null;
  c.donors.forEach(function(d){
    if(dn)return;
    var found=(d.donations||[]).find(function(x){return x.incomeRef===item.id;});
    if(found){donor=d;dn=found;}
  });
  if(!donor){
  ...
  if(dn){
    dn.amt=amt;dn.date=item.date||dn.date;dn.fund=item.fund||dn.fund||'';
  }else{
    donor.donations.push({amt:amt,date:item.date||'',fund:item.fund||'',proj:'',rec:'Yes',ty:'No',rst:'unrestricted',inkind:'No',fmv:0,itemDescription:'',qpq:0,incomeRef:item.id,fromIncomeTab:true,audit:[]});
  }
}
// Removes a stale donation link left over when an income entry that was previously "Link to:
// Donor" gets switched to Customer or Not linked on edit — otherwise the old donation record
// keeps inflating that donor's totals even though the income entry no longer claims to be theirs.
function _unlinkIncomeFromDonor(c,item){
  if(!c||!item)return;
  (c.donors||[]).forEach(function(d){
...
// Ensures a customer contact exists for this income entry's Source name when the bookkeeper
// sets "Link to: Customer" (i-party-type). Customers (features.js ~1769) are a lightweight
// contact list, not a per-gift ledger like donors — so this is just find-or-create, no
// incomeRef bookkeeping needed, and nothing to clean up if the entry is later edited/deleted.
function _syncIncomeToCustomerTab(c,item){
  if(!c||!item)return;
  var custName=(item.name||'').trim();
  if(!custName)return;
  if(!c.customers)c.customers=[];
  var existing=c.customers.find(function(cu){return cu.name.toLowerCase()===custName.toLowerCase();});
  if(!existing){var _newCust4={id:uid(),name:custName,email:'',phone:'',address:'',notes:'',added:new Date().toISOString()};c.customers.push(_newCust4);if(typeof dwUpsertCustomer==='function')dwUpsertCustomer(c,_newCust4);}
}
...
function saveDonation(){
  ...
  var record={id:DONATION_EI>=0?(c.donors[di].donations[DONATION_EI].id||uid()):uid(),amt:amt,date:dateVal,fund:g('dnt-fund').value,proj:g('dnt-proj')&&g('dnt-proj').value||'',rec:g('dnt-rec').value,ty:g('dnt-ty').value,rst:g('dnt-rst').value,inkind:g('dnt-inkind')&&g('dnt-inkind').value||'No',fmv:g('dnt-fmv')&&Number(g('dnt-fmv').value||0)||0,itemDescription:g('dnt-item-desc')&&g('dnt-item-desc').value.trim()||'',inkindFunctional:g('dnt-inkind-func')&&g('dnt-inkind-func').value||'',auctioned:g('dnt-auctioned')&&g('dnt-auctioned').value==='Yes',auctionDate:g('dnt-auction-date')&&g('dnt-auction-date').value||'',auctionSalePrice:g('dnt-auction-sale')&&Number(g('dnt-auction-sale').value||0)||0,auctionBuyerName:g('dnt-auction-buyer')&&g('dnt-auction-buyer').value.trim()||'',qpq:g('dnt-hasqpq')&&g('dnt-hasqpq').value==='Yes'?Number(g('dnt-qpq')&&g('dnt-qpq').value||0):0};
  ...
  if(DONATION_EI<0){
    ...
    if(_dupInc){
      ...
      _dupModal.innerHTML='<div style="background:var(--surface);border-radius:14px;padding:1.5rem;width:380px;box-shadow:0 8px 32px rgba(0,0,0,.2);font-family:\'DM Sans\',sans-serif">'
        +'<div style="font-size:15px;font-weight:600;margin-bottom:.5rem;color:var(--text)"><i class="fas fa-triangle-exclamation"></i> Possible duplicate donation</div>'
  ...
  if(typeof dwUpsertDonation==='function')dwUpsertDonation(c,c.donors[di],record);
...
function renderReports(){
  ...
  var budRows=(c.budgetItems||[]).map(function(b){
    ...
    return '<div class="rpt-row"><span>'+escHtml(b.cat)+' <span style="font-size:10px;color:var(--muted)">('+b.type+')</span></span>'
  ...
  p.innerHTML=''
    ...
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;flex-wrap:wrap;gap:8px">'
      ...
      +'</div>'
      +(hasTagged?'<span style="font-size:11px;color:var(--green)"><i class="fas fa-check"></i> COA-linked</span>':'')
      +'<div style="display:flex;gap:6px">'
      +'<button onclick="openPackagesPanel()" style="font-size:12px;padding:5px 12px;border:1px solid var(--border);border-radius:7px;background:var(--surface);cursor:pointer;font-family:DM Sans,sans-serif;color:var(--text)"><i class="fas fa-box"></i> Packages</button>'
      +'<button onclick="goReportsTab()" style="font-size:12px;padding:5px 12px;border:1px solid var(--border);border-radius:7px;background:var(--surface);cursor:pointer;font-family:DM Sans,sans-serif;color:var(--muted)"><i class="fas fa-xmark"></i> Close</button>'
      +'</div>'
    +'</div>'
    ...
    +'<div id="rpt-header">'
      +'<div class="card">'
        +'<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-bottom:.75rem">'
          +'<div>'+(logoHtml?logoHtml+'<br>':'')+'<span style="font-family:\'DM Serif Display\',serif;font-size:18px;font-weight:400">'+escHtml(c.name)+'</span>'
...
function renderFunctionalExpRpt(){
  ...
  tiles+=tile('Program services',colTotals.program,programPct,'var(--green)',programPct>=75?'<i class="fas fa-check"></i> Charity Navigator standard':'Target: 75%+');
  tiles+=tile('Mgmt & general',colTotals.management,mgmtPct,'var(--text)',mgmtPct<=15?'<i class="fas fa-check"></i> Under 15%':'Consider reviewing');
  ...
  var unclNote=hasUnclassified
    ?'<div style="margin-top:.75rem;padding:.75rem;background:var(--soft);border-radius:8px;font-size:11px;color:var(--amber)"><i class="fas fa-triangle-exclamation"></i> '+colTotals.unclassified>0?fmt(colTotals.unclassified)+' in unclassified expenses. Open each expense and set the Functional field to complete your 990 Part IX.':''+'</div>'
    :'<div style="margin-top:.75rem;font-size:11px;color:var(--green)"><i class="fas fa-check"></i> All expenses classified — ready for Form 990 Part IX.</div>';
...
function _renderExecSummaryBody(el,c){
  ...
  +(c.type==='np'&&grantTotal>0&&iT>0?'<div class="rpt-row"><span style="color:var(--muted)"><i class="fas fa-triangle-exclamation"></i> Grant funds may be restricted</span><span style="color:var(--amber)">'+fmt(grantTotal)+' awarded</span></div>':'')
  ...
  if(budItems.length){
    ...
    +(varLines.length?varLines.slice(0,6).map(function(l){return'<div class="rpt-row"><span>'+escHtml(l.cat)+' <span style="font-size:10px;color:var(--muted)">('+l.type+')</span></span><span style="display:flex;gap:12px"><span style="color:var(--muted)">'+fmt(l.budget)+'</span><span class="'+(l.variance>=0?'vg':'vr')+'">'+(l.variance>=0?'+':'')+fmt(l.variance)+' ('+l.vpct+'%)</span></span></div>';}).join(''):'<div style="font-size:12px;color:var(--muted)">All categories within 10% of budget. Good forecasting.</div>')+'</div>';
  }
  ...
  if(c.type==='np'&&grants.length){
    ...
    h8+=grants.map(function(gr){
      ...
      +(urgency?'<div style="font-size:10px;color:var(--amber);margin-top:.4rem"><i class="fas fa-triangle-exclamation"></i> '+daysLeft+' days left · '+fmt(remaining)+' unspent</div>':'')
...
function runStressTest(){
  ...
  +'</div>'
  +(newNet<0?'<div style="font-size:11px;color:var(--red);margin-top:.5rem;text-align:center"><i class="fas fa-triangle-exclamation"></i> This scenario results in a deficit. Review your cost structure.</div>':newRunway!==null&&newRunway<3?'<div style="font-size:11px;color:var(--amber);margin-top:.5rem;text-align:center"><i class="fas fa-triangle-exclamation"></i> Runway drops below 3 months — consider building reserves.</div>':'<div style="font-size:11px;color:var(--green);margin-top:.5rem;text-align:center"><i class="fas fa-check"></i> Organization remains viable under this scenario.</div>');
...
function renderDonorRpt(){
  ...
  if(lybunt.length){
    ...
    html+='<div style="font-size:12px;font-weight:600;color:#7a5c00;margin-bottom:.375rem"><i class="fas fa-triangle-exclamation"></i> LYBUNT — Gave '+priorFY.label+', not yet '+fy.label+' ('+lybunt.length+')</div>';
  ...
  if(newDonors.length){
    ...
    html+='<div style="font-size:12px;font-weight:600;color:var(--np);margin-bottom:.375rem"><i class="fas fa-seedling"></i> New donors this year ('+newDonors.length+')</div>';
...
function saveMile(){
  ...
  if(typeof dwUpsertMileage==='function')dwUpsertMileage(c,item);
...
var MILE_EI=-1;
function delMile(i){var c=gc();if(!c||!confirm('Delete this mileage entry?'))return;var m=c.mileage[i];c.mileage.splice(i,1);if(m&&typeof dwDeleteMileage==='function')dwDeleteMileage(c,m);sv();renderMileageRpt();}
...
function saveAsset(){
  ...
  var isNewAsset=ASSET_EI<0;
  var itemId=isNewAsset?uid():(c.fixedAssets[ASSET_EI].id||uid());
  var acctCode=_ensureDedicatedCOA(c,name,'Asset','Fixed Assets');
  var item={id:itemId,name:name,date:g('asset-date').value||todayNum(),cost:cost,life:life,salvage:salvage,method:method,annualDepr:Math.round(annual*100)/100,notes:g('asset-notes').value.trim(),acctCode:acctCode};
  if(ASSET_EI>=0)c.fixedAssets[ASSET_EI]=item;else c.fixedAssets.push(item);
  // Dr [asset's dedicated account] / Cr Cash for the purchase — previously never posted at
  // all (see CLARITY_TODO queue item 2 in getCashFlowStatement()'s comment).
  var memo='Fixed asset purchase: '+name;
  if(isNewAsset)postToLedger(c,acctCode,_defaultCashCode(c),cost,memo,'asset',itemId);
  else updateLedgerEntry(c,itemId,acctCode,_defaultCashCode(c),cost,memo,'asset');
  sv();renderAssetsRpt();renderTrialBalance(c);renderGL(c);renderBalanceSheet(c);closeM('m-asset');ASSET_EI=-1;
...
function delAsset(i){var c=gc();if(!c||!confirm('Delete this asset?'))return;if(!c.fixedAssets)return;var a=c.fixedAssets[i];if(a&&a.id)voidLedgerEntry(c,a.id);c.fixedAssets.splice(i,1);sv();renderAssetsRpt();renderTrialBalance(c);renderGL(c);renderBalanceSheet(c);}
...
function renderEstTaxRpt(){
  ...
  el.innerHTML='<div class="rpt-sec"><div style="display:flex;align-items:center;gap:12px;margin-bottom:1rem"><div class="rpt-ttl" style="margin-bottom:0">Estimated Quarterly Taxes</div><span class="badge b-gray" style="font-size:10px">IRS Form 1040-ES</span></div>'
    ...
    +'<div style="margin-top:.75rem;font-size:11px;color:var(--muted);line-height:1.6"><i class="fas fa-triangle-exclamation"></i> This is an estimate using 2024 federal tax brackets for a single filer, based on data entered in Clarity. State income taxes are not included. Consult a CPA before making payments. Safe harbor: pay 100% of prior year tax liability to avoid underpayment penalty.</div>'
    +'</div></div>';
}
...
function renderLoansRpt(){
  ...
  var rows=sched.rows.map(function(r){
    ...
    return'<tr style="'+(isPosted?'opacity:.5':'')+'"><td style="color:var(--muted);font-size:11px">#'+r.num+(r.dueDate?' · '+r.dueDate:'')+'</td>'
      ...
      +(isPosted
        ?'<span class="badge b-green" style="font-size:10px">Posted <i class="fas fa-check"></i></span>'
        :'<button class="add-btn" style="font-size:10px;padding:3px 8px" onclick="postLoanPayment('+_loanRptIdx+','+r.num+','+r.interest.toFixed(2)+','+r.principal.toFixed(2)+')" title="Post this payment to the ledger">Post</button>')
      +'</div></td></tr>';
  }).join('');
  ...
  el.innerHTML='<div class="rpt-sec">'
    ...
    +'<div class="c-title" style="margin-bottom:.75rem"><i class="fas fa-lightbulb"></i> Payoff simulator — extra monthly payment</div>'
    ...
    +(extra>0
      ?'<div style="display:flex;gap:1.5rem;margin-top:.75rem;flex-wrap:wrap">'
        +'<span style="font-size:13px;color:var(--green)"><i class="fas fa-check"></i> Pay off in <strong>'+sched.rows.length+' months</strong> (save '+pymtsSaved+' payments)</span>'
        +'<span style="font-size:13px;color:var(--green)"><i class="fas fa-check"></i> Interest saved: <strong>'+fmt(intSaved)+'</strong></span>'
        +'</div>'
    ...
    +'<div style="display:flex;gap:6px">'
    +'<button class="e-btn" style="border:1px solid var(--border);border-radius:7px;padding:4px 10px;font-size:12px" onclick="editLoan('+_loanRptIdx+')" title="Edit loan">&#9998;</button>'
    +'<button class="d-btn" style="border:1px solid var(--red-bg);border-radius:7px;padding:4px 10px;font-size:12px" onclick="delLoan('+_loanRptIdx+')" title="Delete loan">&#215;</button>'
    +'</div></div>'
    ...
    +'</div>'
    +'<div style="font-size:11px;color:var(--muted);margin-top:.5rem;padding-top:.5rem;border-top:1px solid var(--soft)">Posting a payment books interest as an expense and reduces the loan liability, both posted to the general ledger.</div>'
    +'</div>'
    +'</div>';
}
  ...
  if(transfers.length){
    ...
    html+=transfers.map(function(t,ti){
      return'<tr><td style="color:var(--muted);font-size:11px">'+(t.date||'—')+'</td><td>'+escHtml(t.fromFund)+'</td><td>'+escHtml(t.toFund)+'</td><td class="vb">'+fmt(t.amount)+'</td><td style="font-size:11px;color:var(--muted)">'+escHtml(t.note||'—')+'</td>'
        +'<td><button class="add-btn" style="padding:2px 8px;font-size:11px;background:var(--red);color:#fff;border:none" title="Remove transfer" onclick="deleteTransfer('+ti+')"><i class="fas fa-xmark"></i></button></td></tr>';
    }).join('');
    html+='</tbody></table>';
  }else{
...
function renderBudgetMultiRpt(){
  ...
  var html=Object.keys(groups).map(function(grpName){
    ...
    var rows=items.map(function(b){
      ...
      return'<tr><td style="padding-left:1rem;font-size:12px">'+escHtml(b.cat)+'</td>'+cells+'</tr>';
    }).join('');
...
function renderBudgetTwoYrRpt(){
  ...
  var html=Object.keys(groups).map(function(grpName){
    ...
    var rows=items.map(function(b){
      ...
      return'<tr><td style="padding-left:1rem;font-size:12px">'+escHtml(b.cat)+'</td>'+codeCell+cells+changeCell+'</tr>';
    }).join('');
    var totCells=grpTots.map(function(t){return'<td style="text-align:right;font-weight:500">'+fmt(t)+'</td>';}).join('');
...
function renderBudgetExportRpt(){
  ...
  var html=Object.keys(groups).map(function(grp){
    ...
    var rows=items.map(function(b){return'<div class="rpt-row"><span style="padding-left:1.25rem;color:var(--muted)">'+escHtml(b.cat)+'</span><span>'+fmt(b.amt)+'</span></div>';}).join('');
...
function render1099Report(){
  ...
  var missingAlert=missingTIN>0?'<div style="background:var(--red-bg);color:var(--red);border-radius:8px;padding:.6rem .9rem;font-size:12px;margin-bottom:.75rem"><i class="fas fa-triangle-exclamation"></i> '+missingTIN+' contractor'+(missingTIN>1?'s':'')+' over $600 missing EIN/TIN — edit the expense to add it.</div>':'';
...
function renderHomeWidget(){
  ...
  if(c.type!=='np'){
    ...
    if(_upcoming.length||_overdue.length){
      ...
      _taxAlertHtml='<div style="background:#fff8e1;border:1px solid #f9a825;border-radius:8px;padding:.75rem 1rem;margin-bottom:1rem;font-size:13px;line-height:1.6">'
        +'<div style="font-weight:700;margin-bottom:4px;color:#7a5c00"><i class="far fa-clock"></i> Estimated Tax Reminder</div>'
  ...
  +'<div class="card" style="margin-bottom:0;grid-area:act"><div class="c-head"><span class="c-title"><i class="fas fa-circle-check"></i> Today\'s actions</span><button class="add-btn" onclick="EI=-1;openM(\'m-act\')">+ Add</button></div>'
  +(pending.length?actHtml:'<div style="font-size:12px;color:var(--muted)">No pending actions. What needs to get done today?</div>')
  +(a.filter(function(x){return x.done;}).length?'<div style="font-size:11px;color:var(--green);margin-top:.5rem">'+a.filter(function(x){return x.done;}).length+' completed <i class="fas fa-check"></i></div>':'')
  +'</div>'
  +'<div class="card" style="margin-bottom:0;grid-area:notes"><div class="c-head"><span class="c-title"><i class="fas fa-pen-to-square"></i> Notes</span><button class="add-btn" onclick="EI=-1;openM(\'m-jrn\')">+ Add</button></div>'
...
function _buildCalendarWidget(c){
  ...
  (c.grants||[]).forEach(function(gr){
    if(gr.appDeadline&&gr.status!=='Closed'&&gr.status!=='Denied')addEvent(gr.appDeadline,'Apply: '+gr.name,'#e65100','<i class="fas fa-pen-to-square"></i>','grant');
    if(gr.deadline&&gr.status!=='Closed')addEvent(gr.deadline,'Report: '+gr.name,'#c0392b','<i class="fas fa-clipboard"></i>','grant');
  });
  (c.donors||[]).forEach(function(d){
    (d.interactions||[]).forEach(function(ix){
      if(ix.followupDate&&!ix.completed)addEvent(ix.followupDate,'Follow up: '+d.name+(ix.followupNote?' — '+ix.followupNote:''),'#185FA5','<i class="fas fa-calendar"></i>','donor');
    });
  });
  ...
  for(var dk in calNotes){
    ...
    if(parseInt(parts[0])===year&&parseInt(parts[1])-1===month){
      ...
      (calNotes[dk]||[]).forEach(function(n){
        ...
        _CAL_EVENTS[dayN].push({label:n.text,color:'var(--np)',icon:'<i class="fas fa-thumbtack"></i>',type:'note',id:n.id,dateKey:dk});
      });
    }
  }
  ...
  (c.grants||[]).forEach(function(gr){
    ...
    if(gr.deadline&&gr.status!=='Closed')addUpcoming(gr.deadline,'Report: '+gr.name,'#c0392b','<i class="fas fa-clipboard"></i>');
  });
  (c.donors||[]).forEach(function(d){
    (d.interactions||[]).forEach(function(ix){
      if(ix.followupDate&&!ix.completed)addUpcoming(ix.followupDate,'Follow up: '+d.name+(ix.followupNote?' — '+ix.followupNote:''),'#185FA5','<i class="fas fa-calendar"></i>');
    });
  });
  ...
  for(var dk2 in calNotes){
    ...
    if(days2>=0&&days2<=30)(calNotes[dk2]||[]).forEach(function(n){upcoming.push({days:days2,label:n.text,color:'var(--np)',icon:'<i class="fas fa-thumbtack"></i>'});});
  }
  ...
  return'<div class="card" style="margin-bottom:0;padding:.875rem">'
    ...
    +'<span style="font-size:12px;font-weight:500"><i class="fas fa-calendar"></i> '+mNames[month]+' '+year+'</span>'
...
function saveCalNote(dateKey){
  ...
  var dayN=parseInt(parts[2]);
  if(!_CAL_EVENTS[dayN])_CAL_EVENTS[dayN]=[];
  _CAL_EVENTS[dayN].push({label:text,color:selectedTag.color,icon:'<i class="fas fa-thumbtack"></i>',type:'note',dateKey:dateKey});
  renderHomeWidget();
...
function buildDynMods(type){
  ...
  h+='<div class="overlay" id="m-bank-acct"><div class="modal"><button class="cx" onclick="closeM(\'m-bank-acct\')">&#215;</button><div class="m-title" id="m-bank-acct-title">Bank account</div><div class="fl"><label>Account name *</label><input type="text" id="ba-name" placeholder="e.g. Chase Checking, Savings"></div><div class="fl"><label>Account type</label><select id="ba-type"><option value="checking">Checking</option><option value="savings">Savings</option><option value="money_market">Money market</option><option value="other">Other</option></select></div><div class="fl"><label>Account number (last 4, optional)</label><input type="text" id="ba-last4" placeholder="e.g. 4321" maxlength="4"></div><div class="fl"><label>Next check number (optional)</label><input type="number" id="ba-next-check" placeholder="e.g. 1001" min="1"><div style="font-size:11px;color:var(--muted);margin-top:3px">Set this once to match wherever your physical check stock starts. It auto-advances each time you print a check.</div></div><div class="fl"><label>What does your check page look like?</label><select id="ba-check-format"><option value="voucher2">1 check + 2 vouchers</option><option value="voucher1">1 check + 1 voucher</option><option value="standard3">3 checks, no vouchers</option></select></div>'
    +'<div class="fl" style="border-top:1px solid var(--border);padding-top:.75rem;margin-top:.25rem"><label>Check print alignment</label><div style="font-size:11px;color:var(--muted);margin-bottom:.5rem">Set your printer to <strong>Actual size / 100%</strong> with no auto-fit before printing — that\'s the most common cause of misalignment, more than these offsets. Then print a test page, hold it against your actual check stock, and nudge the arrows until a real check prints in the right spot.</div>'
    +'<input type="hidden" id="ba-check-offx" value="0"><input type="hidden" id="ba-check-offy" value="0">'
    +'<div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap"><div style="text-align:center">'
    +'<button type="button" class="add-btn" style="padding:4px 10px" onclick="_nudgeCheckOffset(\'y\',-0.05)" title="Nudge up">&uarr;</button>'
...
function saveGrant(){
  ...
  if(isAwarded&&hasAmt&&!wasAwarded&&!existingInc){
    ...
    if(typeof dwUpsertIncome==='function')dwUpsertIncome(c,incEntry);
  ...
  if(typeof dwUpsertGrant==='function')dwUpsertGrant(c,item);
...
function saveInc(){var c=gc();if(!c.income)c.income=[];
  ...
  var cat=g('i-c').value;var _iRecurEnd=g('f-rec-end')&&g('f-rec-end').value.trim()||'';var _iRecurCnt=g('f-rec-count')&&Number(g('f-rec-count').value)||0;var _iPartyType=g('i-party-type')&&g('i-party-type').value||'';var item={id:_rEI>=0?(c.income[_rEI].id||uid()):uid(),name:n,cat:cat,status:g('i-s').value,proj:Number(g('i-p').value||0),recv:Number(g('i-r').value||0),date:g('i-dt')&&g('i-dt').value||'',fund:g('i-fund')&&g('i-fund').value||'',acctCode:g('i-acct')&&g('i-acct').value||lookupAcctByCAT(c,cat,'Income')||'',recurring:g('f-rec').value,grantId:g('i-gid')&&g('i-gid').value||'',partyType:_iPartyType};
  ...
  if(_rEI>=0)updateLedgerEntry(c,item.id||c.income[_rEI].id,_defaultCashCode(c),item.acctCode||'4010',Number(item.recv||0),item.name||'Income','income');else postToLedger(c,_defaultCashCode(c),item.acctCode||'4010',Number(item.recv||0),item.name||'Income','income',item.id);markDirty('funding','budget','reports','bs');if(_rEI>=0)c.income[_rEI]=item;else c.income.push(item);
  if(item.partyType==='donor')_syncIncomeToDonorTab(c,item);
  else if(item.partyType==='customer'){if(_oldInc&&_oldInc.donationRef)_unlinkIncomeFromDonor(c,item);_syncIncomeToCustomerTab(c,item);}
  else if(_oldInc&&_oldInc.donationRef)_unlinkIncomeFromDonor(c,item);
  sv();dwUpsertIncome(c,item);var _pfunding=g('p-funding');if(_pfunding&&_pfunding.classList.contains('active'))renderNpInc(c);renderBudgetMultiYear();renderReports();renderBalanceSheet(c);if(item.grantId){var _pgrantsInc=g('p-grants');if(_pgrantsInc&&_pgrantsInc.classList.contains('active'))renderGrants(c);}var _rp=g('p-recon');if(_rp&&_rp.classList.contains('active'))renderReconciliation(c);var _pdonors=g('p-donors');if(_pdonors&&_pdonors.classList.contains('active'))renderDonors(c);_refreshBankIfActive(c);closeM('m-inc');['i-n','i-c','i-p','i-r','i-dt','i-fund','i-gid','i-party-type'].forEach(function(id){var el=g(id);if(el)el.value='';}); }
function saveRev(){var c=gc();if(!c.revenue)c.revenue=[];
  ...
  if(_rREI>=0){
    // Edit: supersede old ledger entries and repost. The sales-tax sub-entry
    // (sourceId=_existingId+':stax') must ALSO go through updateLedgerEntry, not a raw
    // postToLedger — postToLedger has no dedup of its own (that's what updateLedgerEntry is
    // for), so every edit to a taxed revenue item was stacking a brand-new Sales Tax Payable
    // entry on top of the previous one instead of superseding it, permanently overstating the
    // liability by every prior edit's tax amount. Also: if tax is removed on this edit
    // (_taxAmt now 0), the old tax entry must be voided, not silently left active — the
    // previous code just skipped the whole tax branch in that case, leaving it stale forever.
    ...
    if(_taxAmt>0)updateLedgerEntry(c,_existingId+':stax',_defaultCashCode(c),_stCode,_taxAmt,(item.name||'Revenue')+' — sales tax collected','revenue');
    else voidLedgerEntry(c,_existingId+':stax');
  }else{
  ...
  markDirty('revenue','budget','reports','bs');if(_rREI>=0)c.revenue[_rREI]=item;else c.revenue.push(item);sv();dwUpsertRevenue(c,item);var _prev=g('p-revenue');if(_prev&&_prev.classList.contains('active'))renderRev(c);renderBudgetMultiYear();renderReports();renderBalanceSheet(c);var _rp=g('p-recon');if(_rp&&_rp.classList.contains('active'))renderReconciliation(c);_refreshBankIfActive(c);closeM('m-rev');['r-n','r-c','r-p','r-a','r-dt','r-taxrate','r-taxamt','r-cust'].forEach(function(id){var el=g(id);if(el)el.value='';});if(g('r-taxrate'))g('r-taxrate').value='0';if(g('r-taxjur'))g('r-taxjur').value=''; }
function revTaxCalc(){
...
function savePeInc(){var c=gc();if(!c.income)c.income=[];var n=g('pi-n').value.trim();if(!n)return;var _rPEI=resolveEI(c.income);var item={id:_rPEI>=0?(c.income[_rPEI].id||uid()):uid(),name:n,cat:g('pi-c').value,amt:Number(g('pi-a').value||0),freq:g('pi-f').value,date:g('pi-d').value,acctCode:g('pi-acct')&&g('pi-acct').value||'',recurring:g('f-rec').value};markDirty('peinc','budget','reports','bs');
  // DOUBLE ENTRY: Dr Cash / Cr Income — this was previously missing entirely for Personal
  // clients (unlike NP's saveInc()/SB's saveRev(), which always posted), so PE income never
  // showed up on the General Ledger or Trial Balance tabs even though PE expenses did.
  if(_rPEI>=0)updateLedgerEntry(c,item.id,_defaultCashCode(c),item.acctCode||'4010',Number(item.amt||0),item.name||'Income','income');
  else postToLedger(c,_defaultCashCode(c),item.acctCode||'4010',Number(item.amt||0),item.name||'Income','income',item.id);
  if(_rPEI>=0)c.income[_rPEI]=item;else c.income.push(item);sv();dwUpsertIncome(c,item);renderPeInc(c);renderBudgetMultiYear();renderReports();closeM('m-peinc');['pi-n','pi-c','pi-a','pi-d'].forEach(function(id){g(id).value='';}); }
...
function syncVendorFromExpense(c,item){
  ...
  var _newVendor2={id:uid(),name:name,defaultCat:item.cat||'',defaultAcctCode:item.acctCode||'',is1099:!!item.is1099,tin:item.tin1099||'',email:'',phone:'',address:'',notes:''};
  c.vendors.push(_newVendor2);
  if(typeof dwUpsertVendor==='function')dwUpsertVendor(c,_newVendor2);
}
...
// Re-checks the Bank tab's pending transactions against the books, but only
// if the Bank tab is currently the active panel (same pattern already used
// for p-recon below). This exists because _bankFindMatchCandidates() only
// ever runs at renderBank() time — if a bookkeeper is sitting on the Bank
// tab and then enters a new expense/income elsewhere without leaving that
// tab, nothing would otherwise tell the pending list to re-check itself
// against the newly-saved entry, so a real match could sit unnoticed.
function _refreshBankIfActive(c){
  var p=g('p-bank');
  if(p&&p.classList.contains('active')&&typeof renderBank==='function')renderBank(c);
}
function saveExp(){
...
var _ACT_ICON={
  'Phone call':'<i class="fas fa-phone"></i>','Meeting':'<i class="fas fa-handshake"></i>','Email':'<i class="fas fa-envelope"></i>','Site visit':'<i class="fas fa-building"></i>','Event':'<i class="fas fa-ticket"></i>','Letter sent':'<i class="fas fa-envelope-open-text"></i>',
  'Proposal sent':'<i class="fas fa-file"></i>','Ask made':'<i class="fas fa-comment"></i>','Pledge received':'<i class="fas fa-clipboard"></i>','Donation received':'<i class="fas fa-heart"></i>',
  'Thank you sent':'<i class="fas fa-envelope"></i>','Impact report sent':'<i class="fas fa-chart-column"></i>','Lapsed — re-engaged':'<i class="fas fa-rotate"></i>','Note':'<i class="fas fa-pen-to-square"></i>'
};
...
function _renderActivityLog(d,di){
  ...
  return'<div style="margin-top:.75rem;border-top:1px solid var(--soft);padding-top:.625rem">'
    ...
    +acts.map(function(a,ai){
      var icon=_ACT_ICON[a.type]||'<i class="fas fa-thumbtack"></i>';
      ...
      return'<div style="position:relative;margin-bottom:.5rem">'
        ...
        +(a.followupDate&&!a.completed?'<button class="add-btn" style="font-size:11px;padding:3px 8px" onclick="completeActivity('+di+','+ai+')"><i class="fas fa-check"></i></button>':'')
        ...
        +(a.followupDate?'<div style="margin-top:3px;font-size:11px;color:'+(a.completed?'var(--green)':overdue?'var(--red)':'var(--amber)')+';">'
          +(a.completed?'<i class="fas fa-check"></i> Completed '+fmtDate(a.completedDate||''):'<i class="fas fa-calendar"></i> Follow up: '+fmtDate(a.followupDate)+(a.followupNote?' — '+escHtml(a.followupNote):'')+(overdue?' — OVERDUE':''))
          +'</div>':'')
        +'</div>'
        +'</div>';
    }).join('')
...
function openActivityModal(di){
  ...
  mo.innerHTML='<div class="modal" style="max-width:480px">'
    ...
    +'<div style="font-size:11px;color:var(--np);padding:.5rem .75rem;background:var(--np-bg);border-radius:7px;margin-top:.25rem" id="act-income-note"><i class="fas fa-check"></i> Will auto-post to Income</div>'
    +'</div>'
  ...
  if(!editId){
    ...
    if(type==='Pledge received'){
      ...
      if(pAmt>0){
        ...
        if(typeof dwUpsertIncome==='function')dwUpsertIncome(c,pledgeItem);
...
function _donForceLog(){
  ...
  if(typeof dwUpsertDonation==='function')dwUpsertDonation(_p.c,_p.c.donors[_p.di],_p.record);
...
function delBudgetLine(i){var c=gc();if(!c||!c.budgetItems[i])return;if(!confirm('Delete "'+c.budgetItems[i].cat+'"?'))return;var bi=c.budgetItems[i];c.budgetItems.splice(i,1);if(typeof dwDeleteBudgetItem==='function')dwDeleteBudgetItem(c,bi);sv();renderBudgetMultiYear();}
function saveBudget(){
  ...
  if(_editItem){
    ...
    if(typeof dwUpsertBudgetItem==='function')dwUpsertBudgetItem(c,_editItem);
  }else{
    var ex=c.budgetItems.find(function(b){return b.cat===cat&&b.type===type;});
    if(ex){ex.amt=amt;ex.group=grp;ex.fund=g('b-fund')&&g('b-fund').value||'';ex.overspendPolicy=g('b-overspend')&&g('b-overspend').value||'warn';if(typeof dwUpsertBudgetItem==='function')dwUpsertBudgetItem(c,ex);}
    else{var _newBudgetItem={cat:cat,type:type,amt:amt,group:grp,fund:g('b-fund')&&g('b-fund').value||'',overspendPolicy:g('b-overspend')&&g('b-overspend').value||'warn'};c.budgetItems.push(_newBudgetItem);syncBudgetToCOA(c,cat,type,grp);if(typeof dwUpsertBudgetItem==='function')dwUpsertBudgetItem(c,_newBudgetItem);}
  }
...
function renderConsolidatedPL(){
  ...
  p.innerHTML='<div class="card">'
    ...
    +'<button class="xbtn" onclick="doPDF(&quot;consolidated&quot;)" style="font-size:11px"><i class="fas fa-arrow-down"></i> Export PDF</button></div>'
    +tiles
...
function renderRecurBanner(){
  ...
  banner.innerHTML='<div style="background:var(--np,#2d6a4f);color:#fff;padding:8px 16px;font-size:12px;display:flex;align-items:flex-start;gap:12px;line-height:1.5;box-shadow:0 2px 8px rgba(0,0,0,.15)">'
    +'<div style="flex:1"><i class="fas fa-rotate"></i> <strong>Recurring transactions</strong> — '
    ...
    +'style="background:rgba(0,0,0,.2);border:none;color:#fff;padding:3px 10px;border-radius:4px;cursor:pointer;font-size:12px;flex-shrink:0;margin-top:1px"><i class="fas fa-xmark"></i> Dismiss</button>'
    +'</div>';
}
