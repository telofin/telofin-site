...
function saveInc(){var c=gc();if(!c.income)c.income=[];
  ...
  if(_rEI>=0)updateLedgerEntry(c,item.id||c.income[_rEI].id,_defaultCashCode(c),item.acctCode||'4010',Number(item.recv||0),item.name||'Income','income');else postToLedger(c,_defaultCashCode(c),item.acctCode||'4010',Number(item.recv||0),item.name||'Income','income',item.id);markDirty('funding','budget','reports','bs');if(_rEI>=0)c.income[_rEI]=item;else c.income.push(item);sv();dwUpsertIncome(c,item);var _pfunding=g('p-funding');if(_pfunding&&_pfunding.classList.contains('active'))renderNpInc(c);renderBudgetMultiYear();renderReports();renderBalanceSheet(c);if(item.grantId){var _pgrantsInc=g('p-grants');if(_pgrantsInc&&_pgrantsInc.classList.contains('active'))renderGrants(c);}var _rp=g('p-recon');if(_rp&&_rp.classList.contains('active'))renderReconciliation(c);_refreshBankIfActive(c);closeM('m-inc');['i-n','i-c','i-p','i-r','i-dt','i-fund','i-gid'].forEach(function(id){var el=g(id);if(el)el.value='';}); }
