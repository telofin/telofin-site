...
function editItem(type,i){
  ...
  if(type==='income'){if(c.type==='np'){g('i-n').value=r.name||'';var _ic=g('i-c');if(_ic)_ic.setAttribute('data-pending-val',r.cat||'');var _is=g('i-s');if(_is)_is.value=r.status||'';g('i-p').value=r.proj||'';g('i-r').value=r.recv||'';g('f-rec').value=r.recurring||'None';if(g('f-rec-end'))g('f-rec-end').value=r.recurEndDate||'';if(g('f-rec-count'))g('f-rec-count').value=r.recurCount||'';_toggleRecurOpts(r.recurring||'None');if(g('i-fund'))g('i-fund').value=r.fund||'';if(g('i-dt'))g('i-dt').value=r.date||'';openM('m-inc');var _delIncBtn=g('del-inc-btn');if(_delIncBtn)_delIncBtn.style.display='';var ibank=g('i-bank');if(ibank){if(r.bankId)ibank.value='bank:'+r.bankId;else if(r.bsAssetId)ibank.value='bsasset:'+r.bsAssetId;else ibank.value='';};}else{g('pi-n').value=r.name||'';var _pic=g('pi-c');if(_pic)_pic.setAttribute('data-pending-val',r.cat||'');g('pi-a').value=r.amt||'';g('pi-f').value=r.freq||'Monthly';g('pi-d').value=r.date||'';g('f-rec').value=r.recurring||'None';if(g('f-rec-end'))g('f-rec-end').value=r.recurEndDate||'';if(g('f-rec-count'))g('f-rec-count').value=r.recurCount||'';_toggleRecurOpts(r.recurring||'None');openM('m-peinc');}}
  else if(type==='revenue'){g('r-n').value=r.name||'';if(g('r-cust'))g('r-cust').value=r.customerName||'';var _rc=g('r-c');if(_rc)_rc.setAttribute('data-pending-val',r.cat||'');g('r-cf').value=r.conf||'Confirmed';g('r-p').value=r.proj||'';g('r-a').value=r.act||'';g('f-rec').value=r.recurring||'None';if(g('f-rec-end'))g('f-rec-end').value=r.recurEndDate||'';if(g('f-rec-count'))g('f-rec-count').value=r.recurCount||'';_toggleRecurOpts(r.recurring||'None');if(g('r-dt'))g('r-dt').value=r.date||'';if(g('r-taxrate'))g('r-taxrate').value=r.taxRate||0;if(g('r-taxamt'))g('r-taxamt').value=r.taxAmt||'';if(g('r-taxjur'))g('r-taxjur').value=r.taxJurisdiction||'';openM('m-rev');var rbank=g('r-bank');if(rbank){if(r.bankId)rbank.value='bank:'+r.bankId;else if(r.bsAssetId)rbank.value='bsasset:'+r.bsAssetId;else rbank.value='';}}  else if(type==='expenses'){if(c.type==='np'){g('e-d').value=r.desc||'';var _ec1=g('e-c');if(_ec1)_ec1.setAttribute('data-pending-val',r.cat||'');g('e-a').value=r.amt||'';g('e-dt').value=r.date||'';g('e-f').value=r.fund||'';var _egidEl=g('e-gid');if(_egidEl)_egidEl.setAttribute('data-pending-gid',r.grantId||'');var _egpctEl2=g('e-gpct');if(_egpctEl2)_egpctEl2.value=r.grantPct!=null?r.grantPct:'';g('f-rec').value=r.recurring||'None';if(g('f-rec-end'))g('f-rec-end').value=r.recurEndDate||'';if(g('f-rec-count'))g('f-rec-count').value=r.recurCount||'';_toggleRecurOpts(r.recurring||'None');if(g('e-ref'))g('e-ref').value=r.checkNum||'';if(g('e-func'))g('e-func').value=r.functional||'';if(g('e-url'))g('e-url').value=r.receiptUrl||'';if(g('e-tin'))g('e-tin').value=r.tin1099||'';if(g('e-vendor'))g('e-vendor').value=r.vendor1099||r.desc||'';if(g('e-1099'))g('e-1099').value=r.is1099?'yes':'';openM('m-exp');var _expTitle=g('m-exp')&&g('m-exp').querySelector('.m-title');if(_expTitle)_expTitle.textContent='Edit expense';var _delExpBtn=g('del-exp-btn');if(_delExpBtn)_delExpBtn.style.display='';var ebank=g('e-bank');if(ebank){if(r.bankId)ebank.value='bank:'+r.bankId;else if(r.ccId)ebank.value='cc:'+r.ccId;else if(r.bsAssetId)ebank.value='bsasset:'+r.bsAssetId;else ebank.value='';};}else if(c.type==='sb'){g('e-d').value=r.desc||'';var _ec2=g('e-c');if(_ec2)_ec2.setAttribute('data-pending-val',r.cat||'');g('e-a').value=r.amt||'';g('e-fr').value=r.freq||'Monthly';g('e-fx').value=r.fixed||'Fixed';g('f-rec').value=r.recurring||'None';if(g('f-rec-end'))g('f-rec-end').value=r.recurEndDate||'';if(g('f-rec-count'))g('f-rec-count').value=r.recurCount||'';_toggleRecurOpts(r.recurring||'None');if(g('e-ref'))g('e-ref').value=r.checkNum||'';if(g('e-url'))g('e-url').value=r.receiptUrl||'';if(g('e-tin'))g('e-tin').value=r.tin1099||'';if(g('e-vendor'))g('e-vendor').value=r.vendor1099||r.desc||'';if(g('e-1099'))g('e-1099').value=r.is1099?'yes':'';openM('m-exp');var _expTitleSb=g('m-exp')&&g('m-exp').querySelector('.m-title');if(_expTitleSb)_expTitleSb.textContent='Edit expense';var _delExpBtnSb=g('del-exp-btn');if(_delExpBtnSb)_delExpBtnSb.style.display='';var ebankSb=g('e-bank');if(ebankSb){if(r.bankId)ebankSb.value='bank:'+r.bankId;else if(r.ccId)ebankSb.value='cc:'+r.ccId;else if(r.bsAssetId)ebankSb.value='bsasset:'+r.bsAssetId;else ebankSb.value='';};}else{g('e-d').value=r.desc||'';var _ec3=g('e-c');if(_ec3)_ec3.setAttribute('data-pending-val',r.cat||'');g('e-a').value=r.amt||'';g('e-fr').value=r.freq||'Monthly';g('e-dt').value=r.date||'';g('f-rec').value=r.recurring||'None';if(g('f-rec-end'))g('f-rec-end').value=r.recurEndDate||'';if(g('f-rec-count'))g('f-rec-count').value=r.recurCount||'';_toggleRecurOpts(r.recurring||'None');if(g('e-ref'))g('e-ref').value=r.checkNum||'';if(g('e-url'))g('e-url').value=r.receiptUrl||'';if(g('e-tin'))g('e-tin').value=r.tin1099||'';if(g('e-vendor'))g('e-vendor').value=r.vendor1099||r.desc||'';if(g('e-1099'))g('e-1099').value=r.is1099?'yes':'';openM('m-exp');var _expTitlePe=g('m-exp')&&g('m-exp').querySelector('.m-title');if(_expTitlePe)_expTitlePe.textContent='Edit expense';var _delExpBtnPe=g('del-exp-btn');if(_delExpBtnPe)_delExpBtnPe.style.display='';var ebankPe=g('e-bank');if(ebankPe){if(r.bankId)ebankPe.value='bank:'+r.bankId;else if(r.bsAssetId)ebankPe.value='bsasset:'+r.bsAssetId;else ebankPe.value='';}}}
...
function pdfHeader(c){
  ...
  return logo+'<h1>'+c.name+'</h1><p style="color:#8a8880;font-size:11px;margin:0 0 20px">'+tl(c.type)+' &nbsp;·&nbsp; Clarity by Telofin™ &nbsp;·&nbsp; '+today()+(c.fiscalYearEnd?' &nbsp;·&nbsp; FY ends '+fyeLabel(c.fiscalYearEnd):'')+'</p>';
}
...
function openPDF(bodyHtml,c,title){
  ...
  w.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>'+c.name+' — '+(title||'Report')+'</title>'+pdfStyles()+'</head><body>');
...
function exportExecutiveSummary(){
  ...
  if(totalAssets>0||totalLiab>0){
    ...
    bs.assets.forEach(function(a){body+='<tr><td>'+a.name+'</td><td class="right">'+fmtN(a.amt)+'</td><td>Asset</td></tr>';});
    body+='<tr class="total"><td>Total Assets</td><td class="right">'+fmtN(totalAssets)+'</td><td></td></tr>';
    bs.liabilities.forEach(function(l){body+='<tr><td>'+l.name+'</td><td class="right">'+fmtN(l.amt)+'</td><td>Liability</td></tr>';});
...
function doPDF(tabOverride){
  ...
  if(type==='pl'){
    ...
    var _f9warn=_f9untagged?'<p style="color:#c0392b;font-size:11px;margin-bottom:8px">⚠ '+_f9untagged+' expense(s) have no functional classification and have been defaulted to Management & General. Tag expenses using the "Functional" field when recording them.</p>':'';
...
function renderBudgetMultiYear(){
  ...
  if(BUDGET_VIEW==='proposed'){
    ...
    var fyTabs=['FY '+(curYear+1),'FY '+(curYear+2),'FY '+(curYear+3)].map(function(fyl){
      ...
      return'<button class="'+(PROPOSED_FY===fyl?'sv-btn':'add-btn')+'" style="font-size:11px;padding:4px 12px" onclick="PROPOSED_FY=\''+fyl+'\';renderBudgetMultiYear()">'+fyl+(hasDraft?' ✎':'')+'</button>';
    }).join('');
    ...
    +(propItems.length?'<button class="sv-btn" style="margin-left:1rem;background:var(--green);font-size:12px;padding:6px 14px" onclick="adoptBudget()">✓ Adopt '+PROPOSED_FY+'</button>':'')+'</div>'
...
function renderAR(c){
  ...
  var rows=filt.map(function(inv,fi){var oi=invs.indexOf(inv);var od=inv.status!=='Paid'&&inv.due&&isPast(inv.due);var dp=od?daysPast(inv.due):0;var ageLbl=od?(dp>90?'<span class="badge b-red">90+ days</span>':dp>60?'<span class="badge b-red">61–90d</span>':dp>30?'<span class="badge b-amber">31–60d</span>':'<span class="badge b-amber">1–30d</span>'):'';return'<tr><td style="font-weight:500">'+(inv.num||'—')+'</td><td>'+escHtml(inv.client||'—')+'</td><td>'+escHtml(inv.desc||'—')+'</td><td>'+fmt(inv.amt)+'</td><td style="color:var(--muted)">'+(inv.date||'—')+'</td><td style="color:'+(od?'var(--red)':'var(--muted)')+'">'+(inv.due||'—')+(od?' ⚠':'')+' '+ageLbl+'</td><td>'+SB(inv.status||'Draft')+'</td><td><div class="row-acts">'+(inv.status!=='Paid'?'<button class="e-btn" onclick="markInvPaid('+oi+')" title="Mark paid">✓</button><button class="e-btn" onclick="copyPayReminder('+oi+')" title="Copy payment reminder">📋</button><button class="e-btn" onclick="emailInv('+oi+')" title="Send via email">✉</button>'+(od?'<button class="e-btn" onclick="writeBadDebt('+oi+')" title="Write off as bad debt" style="color:var(--red)">✕ Bad debt</button>':''):'')+'<button class="e-btn" onclick="editInv('+oi+')" title="Edit">&#9998;</button><button class="d-btn" onclick="delInv('+oi+')" title="Delete">&#215;</button></div></td></tr>';}).join('');
...
function markInvPaid(i){
  ...
  var inv=c.invoices[i];
  var revItem={id:uid(),name:'Invoice: '+(inv.client||inv.num),cat:'Invoiced Revenue',proj:inv.amt,act:inv.amt,conf:'Confirmed',recurring:'None',invoiceId:inv.id,acctCode:lookupAcctByCAT(c,'Revenue')||''};
...
function renderJournalEntries(c){
  ...
  var rows=filt.map(function(e,fi){var oi=entries.indexOf(e);return'<tr><td style="color:var(--muted)">'+(e.date||'—')+'</td><td>'+SB(e.type||'Other')+'</td><td style="font-weight:500">'+(e.memo||'—')+'</td><td>'+(e.debitAcct||'—')+'</td><td>'+(e.creditAcct||'—')+'</td><td>'+fmt(e.amt)+'</td><td><div class="row-acts"><button class="e-btn" onclick="editJE('+oi+')">&#9998;</button><button class="d-btn" onclick="delJE('+oi+')">&#215;</button></div></td></tr>';}).join('');
...
function recordPartialPayment(i){
  ...
  if(!c.revenue)c.revenue=[];
  var revItem={id:uid(),name:'Partial payment — Inv #'+(inv.num||inv.id)+' ('+(inv.client||'')+')',cat:'Invoiced Revenue',proj:0,act:pmtAmt,conf:'Confirmed',recurring:'None',invoiceId:inv.id,date:todayNum(),acctCode:lookupAcctByCAT(c,'Revenue')||'4010'};
  c.revenue.push(revItem);
...
function renderBalanceSheet(c){
  ...
  function autoRow(label,amt,note){
    return '<tr><td style="color:var(--muted);font-style:italic">'+label+'<span style="font-size:10px;margin-left:6px;background:var(--soft);padding:1px 5px;border-radius:4px;">auto</span>'+(note?'<div style="font-size:10px;color:var(--muted)">'+note+'</div>':'')+'</td><td>'+fmt(amt)+'</td><td></td></tr>';
  }
  ...
  if(BS_VIEW==='ledger'){
    ...
    ledgerHtml='<div style="font-size:11px;color:var(--muted);margin-bottom:.75rem;line-height:1.6">'
      ...
      +'A = L + E check: '+fmt2(lbs.totalAssets)+' = '+fmt2(lbs.totalLiab)+' + '+fmt2(lbs.totalEquity)+' + '+fmt2(lbs.netIncome)+' (net income) → '+(lbs.balanced?'✓ Balanced':'⚠ Difference: '+fmt2(Math.abs(lbs.totalAssets-(lbs.totalLiab+lbs.totalEquityPlusIncome))))
      +'</div>'
...
function renderClosingEntries(c){
  ...
  if(alreadyClosed){
    return'<div class="card" style="border-left:3px solid var(--green)">'
      ...
      +'<div style="padding:.75rem 1rem;font-size:12px;color:var(--muted)">Closing entries for <strong>'+priorFYLabel+'</strong> were posted on <strong>'+alreadyClosed.postedOn+'</strong>. Income and expense accounts have been zeroed to <strong>'+retainedCode+' '+retainedAcct.name+'</strong>.</div>'
      +historyHtml
      +'</div>';
  }
...
function renderReconciliation(c){
  ...
  if(!coaBanks.length&&!hasRealBank&&!hasAnyCleared){
    onboardHtml='<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1.25rem">'      +'<div style="font-size:14px;font-weight:600;margin-bottom:.35rem">Set up reconciliation</div>'      +'<div style="font-size:12px;color:var(--muted);line-height:1.7;margin-bottom:.875rem">'      +'Reconciliation is manual — no live bank connection. You work from your actual bank statement and check off transactions here.<br><br>'      +'<strong style="color:var(--text)">Tip:</strong> If you already set up bank or cash accounts in your Chart of Accounts, they appear here automatically. Otherwise add one below.<br><br>'      +'<strong style="color:var(--text)">1.</strong> Add your bank account (or check your COA — it may already be there)<br>'      +'<strong style="color:var(--text)">2.</strong> Pull up your bank statement from your bank\'s website<br>'      +'<strong style="color:var(--text)">3.</strong> Enter opening and closing balances, then check off each transaction'      +'</div>'      +'<div style="display:flex;gap:.5rem;flex-wrap:wrap">'      +'<button onclick="var b=document.querySelector(\'[data-panel=bank]\');if(b)switchTab({target:b},\'bank\')" style="padding:7px 16px;border:none;border-radius:7px;background:var(--np);color:#fff;font-size:12px;font-weight:500;cursor:pointer">🏦 Go to Bank tab to import</button>'      +'</div></div>';
  }

  var _bankLink='<button class="add-btn" style="font-size:11px;padding:4px 10px" onclick="var b=document.querySelector(\'[data-panel=bank]\');if(b)switchTab({target:b},\'bank\')">🏦 Import via Bank tab</button>';
  var acctPicker=allAccts.length>1?'<div style="display:flex;align-items:center;gap:8px;margin-bottom:1rem;flex-wrap:wrap"><span style="font-size:12px;color:var(--muted)">Account:</span><div class="sw"><select onchange="RECON_ACCT=this.value;renderReconciliation(gc())">'+allAccts.map(function(a){return'<option value="'+a.id+'"'+(RECON_ACCT===a.id?' selected':'')+'>'+a.name+'</option>';}).join('')+'</select></div>'
  +_bankLink
  +'</div>':'<div style="margin-bottom:.75rem;display:flex;gap:8px;align-items:center"><span style="font-size:12px;color:var(--muted)">Account: <strong>'+selAcct.name+'</strong></span>'+_bankLink+'</div>';
...
function printReconStatement(){
  ...
  var statusBadge=Math.abs(diff)<1
    ?'<span style="background:#d4edda;color:#155724;padding:3px 10px;border-radius:6px;font-size:11px;font-weight:600">✓ RECONCILED</span>'
    :'<span style="background:#fff3cd;color:#856404;padding:3px 10px;border-radius:6px;font-size:11px;font-weight:600">⚠ DIFFERENCE: '+fmtN(Math.abs(diff))+'</span>';
  ...
  body+='<div><div style="font-weight:600;font-size:14px">'+escHtml(selAcct.name)+'</div><div style="color:#888;font-size:12px">'+c.name+'</div>'+(rs.periodStart||rs.periodEnd?'<div style="color:#888;font-size:11px;margin-top:2px">Period: '+(rs.periodStart||'—')+' to '+(rs.periodEnd||'—')+'</div>':'')+'</div>';
...
function reconPullMissing(){
  ...
  div.innerHTML='<div class="overlay open" id="recon-pull-modal" style="z-index:10001">'
    ...
    +'<div><div style="font-size:15px;font-weight:600">⬇ Pull in missing transactions</div>'
    ...
    +missingExp.map(function(e){
      ...
      var src=e.fromBank?'🏦 bank import':e.checkNum?'check #'+e.checkNum:'manual';
    ...
    +missingInc.map(function(r){
      ...
      var srcLabel=r.fromBank?'🏦 bank import':'manual';
      return '<tr style="border-bottom:1px solid var(--border)">'
